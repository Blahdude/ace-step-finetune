...
class ACEStepPipeline:
    def __init__(
        ...
        **kwargs,
        ...
        # Set a clean initial state for LoRA
        self.lora_path = "none"
    ...
    def get_checkpoint_path(self, checkpoint_dir, repo):
        ...
        if checkpoint_dir_models is None:
            if checkpoint_dir is None:
                ...
                checkpoint_dir_models = snapshot_download(repo, cache_dir="/nfs/turbo/smtd-hwdong/ocamp/hf_cache")
    ...
    import os
    from huggingface_hub import snapshot_download
    # Make sure 'logger' and 'set_weights_and_activate_adapters' are defined or imported in your file.

    def load_lora(self, lora_name_or_path, lora_weight):
        """
        Loads, unloads, or changes the weight of a LoRA adapter.

        This function is idempotent (safe to call multiple times) and handles both
        local directory paths and Hugging Face Hub repository IDs.
        """
        # Do nothing if the requested adapter and weight are already active.
        if lora_name_or_path == self.lora_path and lora_weight == self.lora_weight:
            return

        # --- Case 1: Unload the LoRA adapter ---
        # This is triggered when the new path is "none" or None.
        if lora_name_or_path == "none" or lora_name_or_path is None:
            if self.lora_path != "none" and self.lora_path is not None:
                logger.info(f"Unloading LoRA adapter: {self.lora_path}")
                self.ace_step_transformer.unload_lora()
            # Update state to reflect that no LoRA is loaded.
            self.lora_path = "none"
            self.lora_weight = 0.0
            return

        # --- Case 2: Load or Reload a LoRA adapter ---

        # Step 2.1: Determine the local path to the LoRA files (download if needed).
        if os.path.isdir(lora_name_or_path):
            lora_local_path = lora_name_or_path
            logger.info(f"Found LoRA at local directory: {lora_local_path}")
        else:
            logger.info(f"Local directory not found. Attempting to download from Hugging Face Hub: '{lora_name_or_path}'")
            try:
                # Safely get the cache directory attribute or use a default.
                cache_dir = getattr(self, "checkpoints_cache_dir", "/home/ocamp/.cache/ace-step/checkpoints")
                lora_local_path = snapshot_download(lora_name_or_path, cache_dir=cache_dir)
            except Exception as e:
                logger.error(f"Failed to download or find LoRA from '{lora_name_or_path}'. Aborting load. Error: {e}")
                return

        # Step 2.2: Unload any previously loaded adapter to ensure a clean state.
        if self.lora_path != "none" and self.lora_path is not None:
            logger.info(f"Unloading previous adapter '{self.lora_path}' before loading new one.")
            self.ace_step_transformer.unload_lora()

        # Step 2.3: Load the new adapter from the local path.
        adapter_file = os.path.join(lora_local_path, "pytorch_lora_weights.safetensors")

        if not os.path.exists(adapter_file):
            logger.error(f"Adapter file not found at expected path: {adapter_file}")
            return

        logger.info(f"Loading adapter from: {adapter_file}")
        self.ace_step_transformer.load_lora_adapter(
            adapter_file,
            adapter_name="ace_step_lora"  # Use a consistent name for the adapter
        )

        # Step 2.4: Set the weight for the newly loaded adapter and activate it.

        # --- CRITICAL DEBUGGING CHECK ---
        print("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
        print(f"DEBUG CHECK: About to apply LoRA weight: {lora_weight} (type: {type(lora_weight)})")
        print("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")

        set_weights_and_activate_adapters(self.ace_step_transformer, ["ace_step_lora"], [lora_weight])

        # Step 2.5: Update the internal state.
